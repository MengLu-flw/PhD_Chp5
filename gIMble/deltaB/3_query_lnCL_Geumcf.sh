...
time gimble query -z /home/mlu/scratch/2_gimble/2024_May/Geum_cf_sym/Geum_cf_Gstore.z -l gridsearch/windows_kmax2/IM_BA_deltaB_240902
...
####---------------------------- Edit bed output --------------------------------------#
####-------------------------------------------------------------------------------------#
cut -f1-5 Geum_cf_Gstore.gridsearch_windows_kmax2_IM_BA_grid_240610_0.sliced_param.me.bed > Geum_cf_grid_me0.txt
# the first part of the delta_B equation
cut -f10 Geum_cf_Gstore.gridsearch_windows_kmax2_IM_BA_deltaB_240902_0.best.bed > Geum_cf_grid_meFIX.txt
# the second part of the delta_B equation



paste Geum_cf_grid_me0.txt Geum_cf_grid_meFIX.txt > Geum_cf_grid_deltaB.txt # This output contains the information for R plotting

##--- Split into per CHROM
time awk 'NR==1 {next} {print > $1".txt"}' Geum_cf_grid_deltaB.txt # Split the information by per chromosome
##--- Rename files
