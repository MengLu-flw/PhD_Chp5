...
### --- Filter sites with the top 50% of the highest diagnostic index
threshold <- quantile(fit_CHORM01$DI$DI, prob = 0.5)
...
a <- plotPolarized(geno, HI, xlab = "")
b <- plotMarkerAxis("/mnt/destiny/sandbox/rbge_mlu/Chp5/diem_v1.4/1_VCF2diem/snp_pos/OX326997.1-includedSites.txt", ChosenSites = fit_CHORM01$DI$DI > threshold)


### --- Save R session and quit
save.image(file = "diem_plot_CHORM01_DI0.5.RData")
